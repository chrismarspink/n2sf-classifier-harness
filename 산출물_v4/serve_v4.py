#!/usr/bin/env python3
"""serve_v4.py — N²SF 분류 서비스 + 온사이트 학습/무중단 핫리로드 (v4, 단일파일 엔진).

표준 라이브러리만. 온프레미스 CPU 서버 진입점. 엔진=classifier_v4.py(3-tier 단일파일).
반환은 v4 상세 JSON(grade·file·decision·findings·shap·explanation …) — GUI/XAI 그대로 소비 가능.
엔드포인트:
  GET  /                 최소 웹 UI(분류·라벨업로드학습·핫리로드)
  GET  /health           상태·현재 모델버전
  POST /classify         {"text":"..."} 또는 {"file":"/path"} → 상세 등급 JSON
  POST /train            {"labels_path":"..."} 백그라운드 학습 시작 → {out}
  GET  /train/status      마지막 학습 상태(진행/완료)
  POST /reload           {"model":"티어/백엔드","path":"models/..."} → 무중단 교체

특징: 학습은 백그라운드 스레드 → 서비스 무중단. 학습 완료 후 자동/수동 핫리로드로 원자적 스왑(다운타임 0).
"""
from __future__ import annotations
import json, os, threading, traceback, time
...
def _train_job(labels_path, base, out, mix, mix_ratio):
    ...
    try:
        ...
        JOB["result"] = res
        if res.get("ok"):
            info = CLF.reload_model(os.path.basename(out), path=out)   # 무중단 핫리로드
...
UI = """<!doctype html><meta charset=utf-8><title>N²SF v4</title>
...
<h1>N²SF 등급분류 v4 · <span id=ver></span></h1>
<fieldset><legend>1. 문서 분류 (상세/XAI)</legend>
...
<input id=mp placeholder="티어(n2sf-official) 또는 models/n2sf-custom-v2">
    ...
    def do_POST(self):
        ...
        if self.path == "/classify":
            ...
            with _lock:
                ...
                out = req.get("out", "models/n2sf-custom-v" + str(int(time.time())))
                threading.Thread(target=_train_job, args=(lp, req.get("base", "models/n2sf-xlmr-official"),
...
def main():
    ...
    print(f"[serve_v4] N²SF 서비스 :{port} · 모델 {CLF.model_version}", flush=True)
