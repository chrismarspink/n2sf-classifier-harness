#!/usr/bin/env python3
"""admin_site.py — N²SF 분류 API 서버 + 온사이트 학습 '관리 콘솔' (v4, 단일파일 엔진).

가정: 분류 엔진은 classifier_v4.py(3-tier 단일파일). 이 콘솔이 그 엔진을 '관리'한다.
메뉴(전부): 대시보드 · 분류 테스트(상세/XAI) · 데이터 관리(라벨 업로드) · 학습(고객데이터)
          · 모델 버전(적용/롤백) · 모니터링 · 설정.
학습·적용은 train_onsite + classifier_v4(무중단 핫리로드)로 **실제 동작**. 통계/설정 일부는 더미.
실행: python admin_site.py  →  http://localhost:8091
"""
...
import classifier_v4 as C
import train_onsite as T

_ROOT = Path(__file__).resolve().parent.parent
_HERE = Path(__file__).resolve().parent
UPLOAD = _ROOT / "onsite_uploads"; UPLOAD.mkdir(exist_ok=True)
CLF = C.N2SFExplainClassifier(model=os.environ.get("N2SF_MODEL", "n2sf-official"),
                              use_fable=os.environ.get("N2SF_FABLE") == "1")
...
def list_models():
    """번들(산출물_v4/models) + 리포(models, 온사이트 학습 산출물) 모델 나열."""
    ms, seen = [], set()
    active = CLF._active["backend"]
    for base in (_HERE / "models", _ROOT / "models"):
        for d in sorted(base.glob("*")) if base.exists() else []:
            if d.name in seen or not (d / "config.json").exists():
                continue
            seen.add(d.name)
            ms.append({"name": d.name, "where": base.name,
                       "size_mb": round(sum(f.stat().st_size for f in d.rglob("*") if f.is_file())/1e6, 1),
                       "active": d.name == active})
    return ms


HTML = """<!doctype html><html lang=ko><meta charset=utf-8><title>N²SF 관리 콘솔 v4</title>
<style>
...
.main{margin-left:200px;padding:22px 28px;max-width:920px}
...
button.g{background:#6b7280}pre{background:#f5f6f8;padding:10px;border-radius:6px;white-space:pre-wrap;font-size:.82rem;max-height:300px;overflow:auto}
...
.badge{display:inline-block;padding:3px 12px;border-radius:6px;font-weight:800;font-size:1rem}
.C{background:#fbe9e7;color:#c0392b}.S{background:#fbf3e2;color:#9a7b1f}.O{background:#e9f3ec;color:#2e7d57}
.hide{display:none}
</style>
<div class=side><h2>🛡 N²SF Admin v4</h2>
...
<div class=card><b>파이프라인</b><br>T1 정규식/키워드 → T2 NER(Presidio+spaCy) → T3 뉴럴(증류) + 튜닝 앙상블
· 오프라인 · §9(개인정보=S, 기밀표지=C) · 단일파일 classifier_v4.py</div></section>

<section id=s_cls class=hide><h1>분류 테스트 (상세/XAI)</h1><div class=card>
<textarea id=c_txt rows=5 placeholder="문서 텍스트"></textarea>
<p><button onclick=cls()>분류</button></p>
<div id=c_sum></div><pre id=c_out></pre></div></section>

<section id=s_data class=hide><h1>데이터 관리 (고객 라벨)</h1><div class=card>
<p>CSV 붙여넣기 (헤더 <code>text,human_label</code> · 라벨=C/S/O). 라벨 기준 = N²SF §9.</p>
...
<p>데이터셋 <select id=t_ds></select> · 베이스 <select id=t_base><option>models/n2sf-xlmr-official</option><option>models/n2sf-xlmr-large</option></select></p>
...
<section id=s_set class=hide><h1>설정</h1><div class=card>
앙상블: soft · 티어가중 rules0.3/ner0.3/neural4.0 · early-exit ON · locale ko<br>
오프라인: HF_HUB_OFFLINE=1 · 모델=로컬(산출물_v4/models) · NER=spaCy 로컬<br>
Fable T4(옵션): <span id=s_fable>-</span> (N2SF_FABLE=1 + ANTHROPIC_API_KEY 일 때만, 기본 off=온프레미스)<br>
<small>런타임 변경은 classify_text(weights=...)/환경변수로(별도).</small></div></section>
</div>
<script>
function nav(x){for(let s of document.querySelectorAll('section'))s.classList.add('hide');
 ...
 if(x=='dash')stat();if(x=='models')lm();if(x=='mon')lg();if(x=='train')fillds();if(x=='set')stat();}
...
async function stat(){let d=await G('/api/status');d_model.textContent=d.model;d_job.textContent=d.job;d_cnt.textContent=d.classified;s_fable.textContent=d.fable?'ON':'off'}
async function cls(){let r=await J('/api/classify',{text:c_txt.value});
 if(r.grade){c_sum.innerHTML=`<span class="badge ${r.grade}">${r.grade} ${r.gradeLabel||''}</span> conf ${r.confidence} · ${(r.decision||{}).decidedByTier||''}<br><small>${r.explanation||''}</small>`}
 c_out.textContent=JSON.stringify(r,0,2)}
...
async function lm(){let d=await G('/api/models');m_tbl.innerHTML='<tr><th>모델</th><th>위치</th><th>MB</th><th></th></tr>'+
 d.map(m=>`<tr><td>${m.active?'✅ ':''}${m.name}</td><td>${m.where}</td><td>${m.size_mb}</td><td><button onclick="rln('${m.name}')">적용</button></td></tr>`).join('')}
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == "/api/status": return self._s(200, {"model": CLF.model_version, "job": JOB["state"],
                                                     "classified": len(LOGS), "fable": CLF.use_fable})
    ...
    def do_POST(self):
        ...
        if p == "/api/classify":
            try:
                ...
                LOGS.append({"t": time.strftime("%H:%M:%S"), "grade": r["grade"], "conf": r["confidence"],
                             "tier": (r.get("decision") or {}).get("decidedByTier"),
        ...
        if p == "/api/train":
            with _lock:
                ...
                threading.Thread(target=_train_job, args=(ds["file"], req.get("base", "models/n2sf-xlmr-official"),
...
def main():
    ...
    print(f"[admin] N²SF 관리 콘솔 v4 http://localhost:{port} · 모델 {CLF.model_version}", flush=True)
