"""data_classifier.py — 문서 → 등급(C/S/O) 분류 단일 소스 모델.

현재 시스템(classifier-svc)의 분류 모델을 **하나의 파일**로 추출·통합한 것이다.
패턴(정규식·deny-list), Presidio + spaCy NER, 신경망(LLM) tier, 점수화·앙상블·
SHAP 기여도, 그리고 파일 → 텍스트 추출(오피스 계열 + HWPX 우선)까지 전부 포함한다.

핵심 진입점
-----------
    classify(file, n2sf_mode=True, verbose=False, llm_mode=False,
             model="minilm", weights=None, locale="ko") -> dict

반환 JSON 은 "화면에 검출 결과를 출력"하는 데 필요한 정보를 담는다:
    - grade        등급. n2sf_mode=True 면 N²SF 코드 'C'/'S'/'O', False 면 풀네임.
    - score        가중 합산 점수
    - pii          검출된 개인정보 — 타입별 건수(+ verbose 면 실제 검출 항목 텍스트)
    - keywords     등급 키워드(대외비/극비 등) 검출
    - shap         등급 분류 기여도 (feature 별 점수 기여 + 비율)
    - ensemble/tiers/compliance  근거·감사용 메타

파라미터
--------
    file       분류할 파일 경로(str | Path). 오피스(docx/xlsx/pptx) + HWPX 우선,
               txt/csv/md/json/log, pdf(옵션), hwp(옵션) 지원.
    n2sf_mode  True → grade 를 N²SF 3등급 코드(C/S/O)로 표기.
    verbose    True → pii/keywords 에 실제 검출된 원문(text)·offset 포함.
    llm_mode   True → 신경망(LLM) tier 활성 (model 로 백엔드 선택).
    model      신경망 백엔드 키: minilm | mdeberta | mbert | koelectra.
    weights    가중치 override dict. 아래 키 지원(모두 선택):
                 {"tier": {"rules":1.0,"ner":1.0,"neural":1.0},
                  "entity": {"KR_RRN": 6.0, ...},
                  "keyword": [{"keyword":"내부자료","weight":1.5,"label":"내부자료"}],
                  "thresholds": {"confidential":5.5,"sensitive":0.75}}
    locale     ko | ja | en | zh-CN | auto.

CLI
---
    python data_classifier.py <file> [--n2sf] [--verbose] [--llm] \
        [--model minilm] [--locale ko] [--weights weights.json]

의존성: presidio-analyzer, spacy(+로케일 모델). llm_mode 는 transformers/
sentence-transformers 추가 필요(옵션). 자세한 내용은 data_classifier.md 참조.
"""
...
def _main(argv=None):
    ...
    p = argparse.ArgumentParser(
        description="문서 → 등급(C/S/O) 분류 단일 소스 모델",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    p.add_argument("file", help="분류할 파일 경로 (docx/xlsx/pptx/hwpx/txt/pdf...)")
    p.add_argument("--n2sf", action="store_true", default=True,
                   help="등급을 N²SF 코드(C/S/O)로 표기 (기본 on)")
    p.add_argument("--no-n2sf", dest="n2sf", action="store_false",
                   help="등급을 풀네임(OPEN/SENSITIVE/CONFIDENTIAL)으로 표기")
    p.add_argument("--verbose", "-v", action="store_true", help="실제 검출 항목(원문) 포함")
    p.add_argument("--llm", action="store_true", help="신경망(LLM) tier 활성")
    p.add_argument("--model", default="minilm", choices=list(NEURAL_BACKENDS.keys()),
                   help="신경망 백엔드")
    p.add_argument("--locale", default="ko", help="ko|ja|en|zh-CN|auto")
    p.add_argument("--ensemble", default="escalate", choices=ENSEMBLE_METHODS)
    p.add_argument("--weights", help="가중치 override JSON 파일 경로")
    p.add_argument("--out", help="결과 JSON 저장 경로 (미지정 시 stdout)")
    ...
    result = classify(args.file, n2sf_mode=args.n2sf, verbose=args.verbose,
                      llm_mode=args.llm, model=args.model, weights=weights,
                      locale=args.locale, ensemble_method=args.ensemble)
    out = json.dumps(result, ensure_ascii=False, indent=2)
    if args.out:
        Path(args.out).write_text(out, encoding="utf-8")
        print(f"saved → {args.out}  (grade={result['grade']}, score={result['score']})")
    else:
        print(out)
