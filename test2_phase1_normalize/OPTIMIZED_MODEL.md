...
|---|---|---|---|
| macro F1 | 0.2772 | **0.8796** | - |
| accuracy | 0.3800 | **0.8800** | - |
| C 재현율(기밀 누락 방지) | 0.7250 | **0.7000** | - |
| C 정밀도 | 0.3867 | **1.0000** | - |
| 과소분류율(유출 위험) | 0.1100 | **0.1200** | - |
| 과대분류율 | 0.5100 | **0.0000** | - |
...
|---|---|---|
| end-to-end p50 | 1259.387 ms | - ms |
| end-to-end p95 | 1267.431 ms | - ms |
...
### CLI
```bash
python data_classifier.py <file> --locale ko --weights weights.json --llm --model mdeberta --ensemble soft
```
...
r = classify("문서.xlsx", n2sf_mode=True, locale="ko",
             weights=weights, llm_mode=True, model="mdeberta", ensemble_method="soft",
             )
