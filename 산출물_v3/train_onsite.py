#!/usr/bin/env python3
"""train_onsite.py — 고객 현장(on-site) 라벨 데이터로 모델 신규 학습 (v3).
...
- 학습 후: classifier_v3.N2SFClassifier.reload_model(new_dir) 로 **무중단 교체**.

CLI:
  python train_onsite.py --labels customer_labels.csv --base models/n2sf-xlmr-large \
...
def train_onsite(labels_path, base="models/n2sf-xlmr-large", out="models/n2sf-custom-v1",
    ...
    res = {"ok": ok, "model_dir": out, "customer_labels": prep["customer"],
           ...
           "next": f"classifier.reload_model('{Path(out).name}', path='{out}')"}
    if progress:
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="온사이트 고객 라벨 재학습")
    ap.add_argument("--labels", required=True, help="고객 라벨 CSV/JSONL (text + C/S/O)")
    ap.add_argument("--base", default="models/n2sf-xlmr-large", help="이어서 학습할 기존 모델(전이)")
