...
| 항목 | 값 |
|---|---|
| 신경망(BERT) 백엔드 | `kcbert` — KcBERT-base (한국어 구어체) |
| 백엔드 종류 | embed |
| HF 모델 | `beomi/kcbert-base` |
| 앙상블 방식 | `max-rank` |
| C(기밀) 임계값 | 5.5 |
| S(민감) 임계값 | 1.5 |
...
|---|---|---|---|
| macro F1 | 0.2327 | **0.4660** | - |
| accuracy | 0.2917 | **0.5389** | - |
| C 재현율(기밀 누락 방지) | 0.6216 | **0.8829** | - |
| C 정밀도 | 0.2805 | **0.4558** | - |
| 과소분류율(유출 위험) | 0.1167 | **0.0361** | - |
| 과대분류율 | 0.5917 | **0.4250** | - |
...
|---|---|---|
| end-to-end p50 | 60.775 ms | - ms |
| end-to-end p95 | 68.127 ms | - ms |
...
### CLI
```bash
python data_classifier.py <file> --locale ko --weights weights.json --llm --model kcbert --ensemble max-rank
```
...
r = classify("문서.xlsx", n2sf_mode=True, locale="ko",
             weights=weights, llm_mode=True, model="kcbert", ensemble_method="max-rank",
             )
